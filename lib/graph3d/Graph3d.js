...
Graph3d.prototype._redrawLegend = function() {
  ...
  if (isSizeLegend === false) {
    ...
    for (y = ymin; y < ymax; y++) {
      ...
      var color;
      if(this.useExpermimentHotMap)
      {
        color = "rgb("+ String( (1.0-f)*255)+",0," + String(f*255) + ")";
      }
      else
      {
        var hue = f * 240;
...
Graph3d.prototype._redrawSurfaceGraphPoint = function(ctx, point) {
  ...
  if (topSideVisible) {
    ...
    if(this.useExpermimentHotMap)
    {
      var gray = (1 - (zAvg - this.zRange.min) * this.scale.z / this.verticalRatio);
      var color = "rgb("+ String( (1.0-gray)*255)+",0," + String(gray*255) + ")";
      fillStyle = color;
...
Graph3d.prototype._onPinch = function (delta) {
  // Duplicated from _onWheel
  var oldLength = this.camera.getArmLength();
  var newLength = oldLength * (1 - delta / 10);
